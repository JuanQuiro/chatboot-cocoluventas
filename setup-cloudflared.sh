#!/bin/bash

# Script para configurar Cloudflare Tunnel (alternativa a ngrok)

GREEN='\033[0;32m'
YELLOW='\033[1;33m'
BLUE='\033[0;34m'
RED='\033[0;31m'
NC='\033[0m' # No Color

echo -e "${BLUE}╔════════════════════════════════════════════════════════╗${NC}"
echo -e "${BLUE}║  🌐 CONFIGURACIÓN CLOUDFLARE TUNNEL (Alternativa)    ║${NC}"
echo -e "${BLUE}╚════════════════════════════════════════════════════════╝${NC}"
echo ""

# Verificar si cloudflared está instalado
if ! command -v cloudflared &> /dev/null; then
    echo -e "${RED}❌ cloudflared no está instalado${NC}"
    echo ""
    echo -e "${YELLOW}📥 Instalando cloudflared...${NC}"
    echo ""
    
    # Detectar sistema operativo
    if [[ "$OSTYPE" == "linux-gnu"* ]]; then
        echo "Descargando cloudflared para Linux..."
        wget -q https://github.com/cloudflare/cloudflared/releases/latest/download/cloudflared-linux-amd64 -O /tmp/cloudflared
        chmod +x /tmp/cloudflared
        sudo mv /tmp/cloudflared /usr/local/bin/cloudflared
        if [ $? -eq 0 ]; then
            echo -e "${GREEN}✅ cloudflared instalado${NC}"
        else
            echo -e "${RED}❌ Error instalando cloudflared. Por favor instálalo manualmente:${NC}"
            echo "   https://developers.cloudflare.com/cloudflare-one/connections/connect-apps/install-and-setup/installation/"
            exit 1
        fi
    else
        echo -e "${YELLOW}Por favor instala cloudflared manualmente:${NC}"
        echo "   https://developers.cloudflare.com/cloudflare-one/connections/connect-apps/install-and-setup/installation/"
        exit 1
    fi
fi

echo -e "${GREEN}✅ cloudflared encontrado${NC}"
echo ""

# Verificar que el servidor esté corriendo
if ! curl -s http://localhost:3008/api/health > /dev/null; then
    echo -e "${YELLOW}⚠️  El servidor Node.js no está corriendo en el puerto 3008${NC}"
    echo -e "${YELLOW}   Iniciando servidor...${NC}"
    ./start-production.sh > /dev/null 2>&1 &
    sleep 5
fi

# Verificar que el servidor responda
if curl -s http://localhost:3008/api/health > /dev/null; then
    echo -e "${GREEN}✅ Servidor Node.js está corriendo${NC}"
else
    echo -e "${RED}❌ No se pudo iniciar el servidor. Por favor inicia manualmente con: ./start-production.sh${NC}"
    exit 1
fi

echo ""
echo -e "${BLUE}🚀 Iniciando Cloudflare Tunnel...${NC}"
echo ""
echo -e "${YELLOW}⚠️  IMPORTANTE:${NC}"
echo -e "${YELLOW}   1. cloudflared mostrará una URL pública (ej: https://abc-123.trycloudflare.com)${NC}"
echo -e "${YELLOW}   2. Copia esa URL y úsala para configurar el webhook en Meta${NC}"
echo -e "${YELLOW}   3. La URL del webhook debe ser: https://abc-123.trycloudflare.com/webhooks/whatsapp${NC}"
echo ""
echo -e "${YELLOW}📋 Pasos para configurar en Meta:${NC}"
echo -e "${YELLOW}   1. Ve a https://developers.facebook.com/${NC}"
echo -e "${YELLOW}   2. Selecciona tu app de WhatsApp${NC}"
echo -e "${YELLOW}   3. Ve a WhatsApp > Configuration${NC}"
echo -e "${YELLOW}   4. En Webhook, configura:${NC}"
echo -e "${YELLOW}      - Callback URL: [LA URL DE CLOUDFLARED]/webhooks/whatsapp${NC}"
echo -e "${YELLOW}      - Verify Token: [EL TOKEN DE TU .env]${NC}"
echo -e "${YELLOW}   5. Marca los campos: messages y message_status${NC}"
echo ""
echo -e "${GREEN}Presiona Ctrl+C para detener cloudflared${NC}"
echo ""

# Iniciar cloudflared
cloudflared tunnel --url http://localhost:3008


//! Rust API Híbrida - Optimizada para métricas y control
//! Integración con Node.js para funcionalidad completa
...
struct AppState {
    ...
    auth_token: String,
    node_api_url: String, // URL del API Node.js
    http_client: Client,
}
...
async fn main() -> anyhow::Result<()> {
    ...
    info!("🚀 Cocolu Bot - Rust Hybrid API v6.0.0");
    info!("🔗 Integración con Node.js para funcionalidad completa");
    ...
    info!("🔗 Node.js API expected on {}", state.node_api_url);
...
async fn health(State(state): State<AppState>) -> Json<HealthResponse> {
    ...
    let rust_metrics = RustMetrics {
        ...
        memory_mb,
        cpu_percent: 0.0, // TODO: Implementar CPU monitoring
        uptime_secs: uptime,
    };

    // Intentar obtener métricas de Node.js (no bloqueante)
...
async fn combined_health(State(state): State<AppState>) -> Json<MetricsResponse> {
    ...
    let combined = CombinedMetrics {
        ...
        memory_total_mb: memory_mb + 
            (node_health.as_ref()
                .and_then(|_| Some(250)) // Estimado Node.js
                .unwrap_or(0)),
        cpu_total_percent: 0.0, // TODO: Implementar
    };
...
async fn dashboard() -> Html<&'static str> {
    Html(include_str!("../dashboard_hybrid.html"))
}

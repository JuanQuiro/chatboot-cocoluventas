...
function load(){
    fetch('/api/health').then(r=>r.json()).then(d=>{
        ...
        const html=s.map(x=>\`<div class="seller-card"><div class="seller-name">\${x.name}</div><span class="seller-badge \${x.status==='available'?'active':'inactive'}">\${x.status==='available'?'✅ Activo':'❌ Inactivo'}</span><div class="seller-info">ID: \${x.id}</div><div class="seller-info">Rating: ⭐\${x.rating.toFixed(1)}</div><div class="seller-metrics"><div class="metric"><div class="metric-value">\${x.currentClients}</div>Clientes</div><div class="metric"><div class="metric-value">\${Math.round(x.currentClients*20)}%</div>Carga</div></div><button class="btn btn-primary" onclick="openEdit('\${x.id}','\${x.name}','\${x.status}')">✏️ Editar</button><button class="btn \${x.status==='available'?'btn-danger':'btn-success'}" onclick="toggleSellerStatus('\${x.id}','\${x.status}')">\${x.status==='available'?'🔴 Desactivar':'🟢 Activar'}</button></div>\`).join('');
...
function saveSeller(){
    ...
    const maxClients=document.getElementById('sellerName').value.trim()||'10';
    const avgResponse=document.getElementById('avgResponse').value.trim()||'0';
    const notes=document.getElementById('sellerNotes').value.trim()||'Sin notas';
    const workStart=document.getElementById('workStart').value.trim()||'N/A';
    const workEnd=document.getElementById('workEnd').value.trim()||'N/A';
    const notificationInterval=document.getElementById('notificationInterval').value.trim()||'30';
    const daysOff=[];
    for(let i=0;i<6;i++){
        if(document.getElementById('dayOff'+i).checked){
            daysOff.push(['Lunes','Martes','Miércoles','Jueves','Viernes','Sábado'][i]);
        }
    }

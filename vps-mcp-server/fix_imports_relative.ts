...
console.log("🔥 FIXING IMPORTS TO RELATIVE PATHS...");

// 1. APP-INTEGRATED.JS (Root: /var/www/cocolu-chatbot/)
...
// 2. AUTH-SIMPLE.ROUTES.JS (in /src/api/)
...
conn.on("ready", () => {
    // Write via Base64 to be safe
    const APP_B64 = Buffer.from(APP_CODE).toString('base64');
    const AUTH_B64 = Buffer.from(AUTH_CODE).toString('base64');

    // We update both files
    const cmd = `
echo "${APP_B64}" | base64 -d > /var/www/cocolu-chatbot/app-integrated.js
echo "${AUTH_B64}" | base64 -d > /var/www/cocolu-chatbot/src/api/auth-simple.routes.js

...
console.log("🔥 FORCING APP OVERWRITE (BANK-GRADE)...");
...
conn.on("ready", () => {
    conn.sftp((err, sftp) => {
        if (err) throw err;
        const s = sftp.createWriteStream('/var/www/cocolu-chatbot/app-integrated.js');
        s.on('close', () => {
            console.log("✅ App Uploaded. Restarting PM2...");

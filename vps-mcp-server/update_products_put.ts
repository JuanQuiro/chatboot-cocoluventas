...
console.log("🔥 UPDATING PRODUCTS FIX (WITH PUT ROUTE)...");
...
router.put('/:id', (req, res) => {
    ...
    try {
        ...
        const check = db.prepare('SELECT id FROM productos WHERE id = ?').get(id);
        if (!check) return res.status(404).json({ error: 'Producto no encontrado' });

        const parts = [];
        const params = [];

        if (nombre !== undefined) { parts.push('nombre = ?'); params.push(nombre); }
        if (precio_usd !== undefined) { parts.push('precio_usd = ?'); params.push(precio_usd); }
        if (stock_actual !== undefined) { parts.push('stock_actual = ?'); params.push(stock_actual); }
        if (sku !== undefined) { parts.push('sku = ?'); params.push(sku); }
        if (categoria_id !== undefined) { parts.push('categoria_id = ?'); params.push(categoria_id); }
        
        // Add updated_at
        parts.push("updated_at = CURRENT_TIMESTAMP");

        if (parts.length === 1) { // Only updated_at
             return res.json({ success: true, message: 'Nada que actualizar' });
        }

        params.push(id);
        const query = \`UPDATE productos SET \${parts.join(', ')} WHERE id = ?\`;
...
conn.on("ready", () => {
    conn.sftp((err, sftp) => {
        ...
        s.on('close', () => {
            console.log("✅ Updated Products Fix with PUT Route. Restarting...");

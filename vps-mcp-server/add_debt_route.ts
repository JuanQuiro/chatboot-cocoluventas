...
console.log("🔥 ADDING GET /:id/debt-summary ROUTE...");
...
// GET /:id/debt-summary - Resumen de deuda
router.get('/:id/debt-summary', (req, res) => {
    ...
    try {
        ...
        // Check client exists
        const client = db.prepare('SELECT id FROM clientes WHERE id = ?').get(id);
        if (!client) {
            return res.status(404).json({ error: 'Cliente no encontrado' });
        }

        // Calculate debts from installments
        // If installments table is empty or doesn't link to client, return 0
        let stats = {
            total_debt: 0,
            overdue_debt: 0,
        ...
        try {
            // Try to sum debt if installments table exists & has client_id
            const debtQuery = \`
                SELECT 
                    SUM(amount - paid_amount) as total,
                    SUM(CASE WHEN due_date < date('now') THEN (amount - paid_amount) ELSE 0 END) as overdue,
                    MIN(CASE WHEN status != 'paid' THEN due_date END) as next_date
                FROM installments 
                WHERE client_id = ? AND status != 'paid'
            \`;
            const result = db.prepare(debtQuery).get(id);
            if (result) {
                stats.total_debt = result.total || 0;
                stats.overdue_debt = result.overdue || 0;
                stats.next_payment_date = result.next_date || null;
            }
        } catch (e) {
            // Ignore if table doesn't exist or schema mismatch, just return 0
            console.warn('[DEBT SUMMARY] Installments query failed (ignorable):', e.message);
        }
        
        res.json(stats);
    } catch (error) {
        console.error('[CLIENTS DEBT] Error:', error);
        res.status(500).json({ error: error.message });
    } finally {
        if (db) try { db.close(); } catch(e) {}
    }
...
// GET /:id - Obtener Cliente por ID (Moved down for route precedence)
...
conn.on("ready", () => {
    conn.sftp((err, sftp) => {
        ...
        s.on('close', () => {
            console.log("✅ Debt Route Uploaded. Restarting...");

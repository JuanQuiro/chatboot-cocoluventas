...
export default function Connection() {
    ...
    const fetchConnectionData = async () => {
        try {
            // Envolver las llamadas en funciones seguras que capturan errores de Axios
            const safeGetQR = async () => {
                try {
                    return await connectionApi.getQR();
                } catch (error) {
                    return null; // Retornar null en lugar de lanzar error
                }
            };

            const safeGetPairingCode = async () => {
                try {
                    return await connectionApi.getPairingCode();
                } catch (error) {
                    return null; // Retornar null en lugar de lanzar error
                }
            };

            // Ahora sí usar Promise.all de forma segura
            const [qrData, pairingData] = await Promise.all([
                safeGetQR(),
                safeGetPairingCode()
            ]);

            // Procesar QR si está disponible
            if (qrData?.qr) {
                setQrCode(qrData.qr);
            }

            // Procesar código de emparejamiento si está disponible
            if (pairingData?.code) {
